...
import static io.github.whippetdb.memory.db.CacheManager.allocateBlock;
import static io.github.whippetdb.memory.db.CacheManager.releaseBlock;
import static io.github.whippetdb.memory.db.CacheManager.scanCache;
...
public class MemMap {
   ...
   public static final int TABLE_BLOCK_SIZE=TABLE_OFF+8*TABLE_SIZE;
...
//System.out.println("seek("+addr+","+level+","+key+","+keyOff+")");
         ...
         switch(type){
            case TYPE_DATA:
               while(addr!=NULL){
                  //TODO: skip level/2 bytes
                  //if(ms.equals(addr+KEY_OFF+level,key,keyOff+level,keySize-level)){
                  if(ms.equals(addr+KEY_OFF,key,keyOff,keySize)){
                           setAddress(addr);
                     return true;
                  }
               ...
               return false;
            case TYPE_TABLE:
               int tableKey=readKeyElement(key, keyOff, level);
      ...
      final void scan(long addr, final Supplier<Boolean> feed){
         ...
         switch(type){
            ...
            case TYPE_TABLE:{
               int p=TABLE_OFF;
               for(int i=0; i<TABLE_SIZE; p+=ADDR_SIZE,i++){
...
//if(DEBUG) System.out.println("  next="+next);
            int tableKey=readKeyElement(ms, addr+KEY_OFF, level); //table key
            int off=TABLE_OFF+(tableKey<<LOG_ADDR_SIZE);
...
//if(DEBUG) System.out.println("  block -> "+blockToString(addr,false));
      ...
      private String blockToString(long addr, boolean recurse){
         ...
         switch(type){
            ...
            case TYPE_TABLE:
               ...
               s+="}\n";
               if(recurse) for(int i=0; i<TABLE_SIZE; i++){
                  long ref=ms.readLong(addr+TABLE_OFF+i*ADDR_SIZE);
                  if(ref!=0){
   ...
   static int readKeyElement(MemIO key, long keyOff, int level) {
      int b = key.readByte(keyOff + (level>>1));
      return (b >> ((level & 1)<<2)) & 0xf;
   }
}
